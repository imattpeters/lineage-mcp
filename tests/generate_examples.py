...
async def generate_example_responses() -> str:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            # ================================================================
            examples.append("=" * 80)
            examples.append("TOOL 3: read_file(file_path, new_session, show_line_numbers, offset, limit)")
            examples.append("=" * 80)
            examples.append("")
            ...
            # ================================================================
            examples.append("=" * 80)
            examples.append("new_session AND clear() DEMO")
            examples.append("=" * 80)
            examples.append("")

            examples.append(">>> read_file('README.md', new_session=True)  # Clears all caches")
            examples.append("")
            result = await read_file("README.md", new_session=True)
            examples.append(result)
            examples.append("")

            # Demonstrate new_session on list_files
            examples.append(">>> list_files('', new_session=True)  # new_session works on list")
            examples.append("")
            result = await list_files("", new_session=True)
            examples.append(result)
            examples.append("")

            # Demonstrate new_session on search_files
            examples.append(">>> search_files('**/*.py', new_session=True)  # new_session works on search")
            examples.append("")
            result = await search_files("**/*.py", new_session=True)
            examples.append(result)
            examples.append("")

            # Demonstrate clear() tool
            examples.append(">>> clear_cache()  # Alternative to new_session=True")
            examples.append("")
    ...
    examples.append("=" * 80)
    examples.append("")
    examples.append("1. list_files(path, new_session) - Directory listing with markdown table")
    examples.append("2. search_files(pattern, path, new_session) - Glob pattern file search")
    examples.append("3. read_file(file_path, new_session, show_line_numbers, offset, limit)")
    ...
    examples.append("4. write_file(file_path, content, new_session) - Create/overwrite files")
    examples.append("5. edit_file(file_path, old_string, new_string, replace_all, new_session) - String replacement")
    examples.append("6. delete_file(file_path, new_session) - Delete files and empty directories")
    examples.append("7. clear_cache() - Clear all session caches (alternative to new_session=True)")
    examples.append("")
    examples.append("All tools (except clear) support the new_session parameter for cache reset after session continuation.")
    ...
    examples.append("Instruction files shown once per folder per session (new_session=True or clear() resets).")
...
if __name__ == "__main__":
    ...
    print("  1. list_files(path, new_session)")
    print("  2. search_files(pattern, path, new_session)")
    print("  3. read_file(file_path, new_session, show_line_numbers, offset, limit)")
    print("  4. write_file(file_path, content, new_session)")
    print("  5. edit_file(file_path, old_string, new_string, replace_all, new_session)")
    print("  6. delete_file(file_path, new_session)")
    print("  7. clear_cache()")
